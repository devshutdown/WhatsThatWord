...
class WordsTableViewController: UITableViewController,UISearchResultsUpdating, UISearchBarDelegate, UISearchControllerDelegate {
    ...
    func updateSearchResultsForSearchController(searchController: UISearchController)
    {
        ...
        let searchPredicate = NSPredicate(format: "kwords CONTAINS[c] %@", searchController.searchBar.text!)
        let array = (WordsArray as NSArray).filteredArrayUsingPredicate(searchPredicate)
    ...
    //**Begin Copy**
    var WordsArray = [NSManagedObject]()
    //**End Copy**
    ...
    override func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        //8) Change to return WordsArray.count
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if (self.resultSearchController.active) {
            ...
            cell.textLabel?.text = person.valueForKey("kwords") as! String?
            ...
            let cell =
                tableView.dequeueReusableCellWithIdentifier("Cell")
                    as UITableViewCell!
            let person = WordsArray[indexPath.row]
            cell.textLabel?.text = person.valueForKey("kwords") as! String?
            cell.detailTextLabel?.text = ">>"
            return cell
        }
    ...
    override func tableView(tableView: UITableView, commitEditingStyle editingStyle: UITableViewCellEditingStyle, forRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if editingStyle == .Delete {
            ...
            if (self.resultSearchController.active) {
                ...
                context.deleteObject(WordsArray[indexPath.row])
            }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        //**Begin Copy**
        if segue.identifier == "UpdateWords" {
            if let destination = segue.destinationViewController as?
                ...
                if (self.resultSearchController.active) {
                    if let SelectIndex = tableView.indexPathForSelectedRow?.row {
                        ...
                        destination.wordsdb = selectedDevice
                        resultSearchController.active = false
                    }
                }
                else {
                    if let SelectIndex = tableView.indexPathForSelectedRow?.row {
                        let selectedDevice:NSManagedObject = WordsArray[SelectIndex] as NSManagedObject
                        destination.wordsdb = selectedDevice
                    }
                }
            }
